...
export default function CategoryPanel({ category, habits, onClose }: CategoryPanelProps) {
  ...
  return (
    <>
      ...
      {/* DRAWER CONTAINER */}
      <div
        className={`fixed inset-y-0 right-0 z-50 w-full sm:w-[680px] bg-surface border-l border-muted/30 shadow-2xl transition-transform duration-200 ease-out transform overflow-y-auto flex flex-col ${
        ...
        <div className="p-12 space-y-10 flex-1">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center p-3.5 ${iconBgClass}`}>
                <CategoryIcon categoryId={category.id} className="w-7 h-7" />
              </div>
              <h2 className="font-display text-3xl font-bold text-foreground">{category.name}</h2>
            </div>
            <button
              onClick={handleClose}
              className="p-2.5 rounded-full text-text-muted hover:text-foreground hover:bg-muted/30 transition-colors duration-200"
          ...
          <div className="bg-[#F2F7F2] dark:bg-[#1A2821] border border-primary-dark/8 dark:border-primary-dark/20 rounded-2xl p-9 flex gap-6 items-center">
            ...
            <div className="w-[160px] shrink-0 relative flex justify-center">
              <img
                src={extra.heroImage}
                alt={category.name}
                width={160}
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-5">
                {extra.benefits.map((b, idx) => (
                  <div key={idx} className="bg-background rounded-2xl p-5 text-center flex flex-col items-center gap-3 border border-muted/20">
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              {habits.map((habit) => {
                ...
                return (
                  <article key={habit.slug} className="bg-background rounded-2xl p-6 flex items-center justify-between gap-5 border border-muted/20">
                    <div className="flex gap-4 items-center min-w-0">
                      <div className="w-12 h-12 rounded-xl bg-surface flex items-center justify-center text-2xl shrink-0 shadow-xs">
                        {habitEmoji}
                      </div>
